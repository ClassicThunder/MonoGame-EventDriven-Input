...
namespace CTInput
{
    public sealed class MonoGameInput : Input
    {
        ...
        private readonly MonoGameCharacterEvents _mgCharacterEvents;

        public MonoGameInput(Game game) : base(game)
        {
            _mouseEvents = new SiMouseEvents();
            _siKeyboardEvents = new SiKeyboardEvents();
            _mgCharacterEvents = new MonoGameCharacterEvents(game);
        }
        ...
        {
            add { _mgCharacterEvents.CharacterTyped += value; }
            remove { _mgCharacterEvents.CharacterTyped -= value; }
        }
